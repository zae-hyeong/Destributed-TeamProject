...
public class SharingApps_Cli_ex3 {
	private	CMClientStub m_clientStub;
	private SharingApps_CliEventHandler m_eventHandler;
	private boolean m_bRun;
	private Scanner m_scan = null;
	
	public SharingApps_Cli_ex3()
	{
		m_clientStub = new CMClientStub();
		m_eventHandler = new SharingApps_CliEventHandler_ex3(m_clientStub);
		m_bRun = true;
	}
	...
	{	
		System.out.println("유저의 App을 서버에 등록중입니다...");
	...
	{
		SharingApps_Cli_ex3 client = new SharingApps_Cli_ex3();
		CMClientStub cmStub = client.getClientStub();
		cmStub.setAppEventHandler(client.getClientEventHandler());
		
		client.StartCM(); // 서버 연결
		client.testLoginDS(); // 로그인
		client.enrollApps("MtoN_Plus#App for MtoN Plus#2# m, n \nExpected result: m+(m+1)+(m+2)+...+n (m, n is num.)");
